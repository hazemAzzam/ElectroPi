...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <Link href={`/product/${product.id}`} className="block h-full">
      <Card size="sm" className="group h-full overflow-hidden transition-all duration-200 hover:-translate-y-0.5 hover:border-foreground/15 hover:shadow-md">
        <CardHeader>
        ...
        <CardFooter className="gap-2">
          ...
          {hasDiscount && (
            <>
              ...
              <span className="ml-auto text-xs font-medium text-destructive">-{Math.round(product.discountPercentage)}%</span>
            </>
          )}
        </CardFooter>
      </Card>
